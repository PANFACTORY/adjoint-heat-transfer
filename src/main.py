...
i = np.arange(nx)[:, None]  # shape (nx,1)
j = np.arange(ny)[None, :]  # shape (1,ny)
...
for t in range(1, nt):
    ...
    u[t], qx, qy = forward(u[t - 1], u[t], qx, qy, k, nx, ny, dx, dy, dt, u0, q0)
...
for t in range(nt - 2, 0, -1):
    ...
    au_tm1, aqx, aqy, ak = reverse(
        au_tm1, au_t, aqx, aqy, ak, u[t - 1], k, nx, ny, dx, dy, dt, u0
    )
...
im_k = ax_k.imshow(k.T, origin="lower", cmap="viridis")
ax_k.set_title("Design variable")
fig.colorbar(im_k, ax=ax_k)
ax_k.axis("off")

im_u = ax_u.imshow(u[nt - 1].T, origin="lower", cmap="viridis")
ax_u.set_title("Temperature")
fig.colorbar(im_u, ax=ax_u)
ax_u.axis("off")

im_au = ax_au.imshow(au_tm1.T, origin="lower", cmap="viridis")
ax_au.set_title("Adjoint Temperature")
fig.colorbar(im_au, ax=ax_au)
ax_au.axis("off")

im_ak = ax_ak.imshow(df.T, origin="lower", cmap="viridis")
